...
public class MessageControllerV2 {
    ...
    public ResponseEntity<Result> leaveMessage(ServletRequest servletRequest, @RequestBody TakeMessageRequest takeMessageRequest){
        String memberId = (String) servletRequest.getAttribute("memberId");
        return ResponseEntity.ok().body(new Result(messageServiceV2.leaveMessage(Long.parseLong(memberId), takeMessageRequest)));
    }
